...
var UploadPic = function (_React$Component) {
  ...
  _createClass(UploadPic, [{
    ...
    value: function inputChange(e) {
      ...
      for (var i = 0; i < length; i++) {
        ...
        this.upload_ajax(files[i]).then(function (data) {
          // responseFileObj
          var fileObj = _this2.setFileObj(data.data, ++index);
          //单次成功回调
          ...
          //是否触发批次上传成功
          _this2.emitBatchSuccess(length, index, files, responseList);
    ...
    key: 'emitBatchSuccess',
    value: function emitBatchSuccess(length, index, files, responseList) {
      if (length != index) return;
      var batchSuccess = this.props.batchSuccess;
    ...
    value: function setLoadingComponent(index, length, type) {
      ...
      if (index == length && type != 'init') {
        setTimeout(function () {
          ...
          _this3.setState(stateObj, function () {});
        }, 500);
      }
    }
  }, {
